...
for (test in experiment)
{
  param <- setupVertical2(depth = df[examples[2], "depth"],
                          ...
                          bprodin = df[examples[2], "benthos"], # 121 + 2.58*df[examples[2], "benthos"], # conversion from benthic production to flux
                          nStages = test)
...
dimnames(df_m) <- list(NULL, c(param$my_names[3 + iFish], "Stages"))
df_stage <- pivot_longer(as.data.frame(df_m), cols = unname(param$my_names[3 + iFish]))
```
...
param <- setupVertical2(depth = df[examples[1], "depth"],
                        ...
                        bprodin = df[examples[1], "benthos"], # 121 + 2.58*df[examples[1], "benthos"],
                        nStages = 9)

df_fishing_smallPel <- setFishing(param, 1, seq(0, 6, 0.2))
df_fishing_largePel <- setFishing(param, 3, seq(0, 0.075, 0.0005))
df_fishing_demersal <- setFishing(param, 5, seq(0, 1.75, 0.01))
```
