...
for (test in experiment)
{
  param <- setupVertical2(depth = df[examples[2], "depth"],
                          ...
                          dfpho = 121 + 2.58*df[examples[2], "benthos"], # conversion from benthic production to flux
                          nStages = test)
...
p5 <- p5a + p5b + plot_layout(design = layout)  &
  theme(plot.background = element_blank(), plot.margin = margin(0, 1, 0, 1, "mm") ) &
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(size = 10))
